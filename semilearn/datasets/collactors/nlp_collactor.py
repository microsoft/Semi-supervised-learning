...
class DataCollatorWithPadding:
    ...
    def __call__(self, features):
        # TODO: need to handle multiple wav
        ...
        for f in features:
            ...
            input_ids = self.tokenizer(f['text'], max_length=self.max_length, truncation=True)['input_ids']
            f_['input_ids'] = input_ids 
            w_features.append(f_)

            if 'text_s' in f:
                input_ids_s = self.tokenizer(f['text_s'], max_length=self.max_length, truncation=True)['input_ids']
                s_features.append({'input_ids':input_ids_s})

            if 'text_s_' in f:
                input_ids_s_ = self.tokenizer(f['text_s_'], max_length=self.max_length, truncation=True)['input_ids']
        ...
        if 'labels' in batch:
            # return batch['idx'], {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']}, batch['labels']
            return {'idx_lb': batch['idx'], 'x_lb': {'input_ids': batch['input_ids'], 'attention_mask': batch['attention_mask']}, 'y_lb': batch['labels']}
        else:
            # if batch['inputs_values'].shape[0] == 2:
...
def get_bert_base_uncased_collacor(max_length=512):
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    collact_fn = DataCollatorWithPadding(tokenizer, max_length=max_length)
    return collact_fn


def get_bert_base_cased_collacor(max_length=512):
