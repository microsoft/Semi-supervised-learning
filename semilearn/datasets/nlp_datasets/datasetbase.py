...
class BasicDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # TODO: We don't have to add x_ulb here, there's nlp_collactor which formats!
        # if self.transform is not None:
        #     x_ulb = self.transform.ToTensor()(sen[0])

        # x_ulb = sen[0]
        # set augmented images
        if self.is_ulb == False:
            return {'idx':idx, 'text':sen[0],'label':target}
            # return {'idx':idx, 'text':sen[0],'label':target, 'x_ulb':sen[0]} 
        else:
            ...
            if self.alg == 'pseudolabel' or self.alg == 'vat':
                ...
                # return {'idx':idx, 'text':sen[0], 'text_s':sen[self.random_choose_sen()], 'x_ulb':sen[0]}
