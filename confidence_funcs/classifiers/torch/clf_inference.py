    ...
    def predict(self, model, dataset, inference_conf={}):
        ...
        with torch.no_grad():
            ...
            for i, data_dict in enumerate(data_loader):
                data = (
                    data_dict["x_lb"] if "x_lb" in data_dict else data_dict["x_ulb_w"]
                )
                ...
                lst_energy_scores.extend(
                    -T * torch.logsumexp(out["logits"] / T, dim=1).cpu().numpy()
                )
